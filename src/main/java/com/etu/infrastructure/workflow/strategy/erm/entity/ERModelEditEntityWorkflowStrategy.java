...
import static com.etu.infrastructure.workflow.service.WorkflowType.ERM_EDIT_ENTITY;

@Component
public class ERModelEditEntityWorkflowStrategy implements ERModelEntityWorkflowStrategy {
    @Autowired
    ...
    private Consumer<ERModelEntity> getSuccessHandler(Stage editorStage) {
        return entityToSave -> {
            erModelFacade.saveEntity(entityToSave);
            editorStage.close();
        };
    }
